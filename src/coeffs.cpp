...
VariableCoeffs calculate_coeffs( // Renamed function
                     ...
                     double els)
{
    // ... (implementation identical to calculate_variable_coeffs_collocation) ...
    ...
    if (B_profile == 1) {
        ...
        double mag = Bz_grid.maxCoeff(); // MATLAB uses max, not max(abs)
    ...
    for(int i=0; i<base_rho.size(); ++i) if (base_rho(i) <= 1e-12 && std::floor(m) != m) base_rho(i) = 1e-12; // Clamp base
    Eigen::MatrixXd rho0_grid = base_rho.array().pow(m);
    Eigen::MatrixXd inv_rho0_grid = rho0_grid.array().unaryExpr([](double v){ return (std::abs(v) > 1e-15) ? 1.0/v : 0.0; });
    Eigen::MatrixXd c1_grid_denom = 1.0 + theta * Y_grid.array();
    Eigen::MatrixXd c1_grid = c1_grid_denom.array().unaryExpr([](double v){ return (std::abs(v) > 1e-15) ? 1.0/v : 0.0; });
    ...
    Eigen::Map<const Eigen::VectorXd> c1_vec(c1_grid.data(), N_tot); Eigen::Map<const Eigen::VectorXd> c2_vec(c2_grid.data(), N_tot);
    Eigen::Map<const Eigen::VectorXd> c3_vec(c3_grid.data(), N_tot); Eigen::Map<const Eigen::VectorXd> c4_vec(c4_grid.data(), N_tot);
    Eigen::Map<const Eigen::VectorXd> c5_vec(c5_grid.data(), N_tot); Eigen::Map<const Eigen::VectorXd> c6_vec(c6_grid.data(), N_tot);
    Eigen::Map<const Eigen::VectorXd> c7_vec(c7_grid.data(), N_tot); Eigen::Map<const Eigen::VectorXd> c8_vec(c8_grid.data(), N_tot);
    Eigen::Map<const Eigen::VectorXd> ux0_vec(ux0_grid.data(), N_tot); Eigen::Map<const Eigen::VectorXd> uz0_vec(uz0_grid.data(), N_tot);

    VariableCoeffs coeffs_out; // Use renamed struct
    coeffs_out.c1 = sparse_diag(c1_vec); coeffs_out.c2 = sparse_diag(c2_vec);
    coeffs_out.c3 = sparse_diag(c3_vec); coeffs_out.c4 = sparse_diag(c4_vec);
    coeffs_out.c5 = sparse_diag(c5_vec); coeffs_out.c6 = sparse_diag(c6_vec);
    coeffs_out.c7 = sparse_diag(c7_vec); coeffs_out.c8 = sparse_diag(c8_vec);
    coeffs_out.ux0 = sparse_diag(ux0_vec); coeffs_out.uz0 = sparse_diag(uz0_vec);
